...
governed_by: [bond:single-home, bond:no-dogma, bond:prove-before-propose, bond:channel-gates]
updated: 2026-06-26
---
...
## §A — The covalent inheritance constraint *(the load-bearing new content — read first)*

The DIP is **generative by design**: a fresh dyad discovers *its own* identity, NON-NEGOTIABLE, and
vocabulary through SPAOR — *"the form follows the craft"* (Commons `AGENT.md` §Plan). So this DNA travels
**only one way**, and the constraint is itself part of the DNA:

> **Offered as a *starting candidate*, never imposed G0.** A dyad inheriting it ratifies-or-falsifies
> each part through its *own* bootstrap friction; what survives *its* n=1 is the DNA it holds. Inherited
> by transfer-as-G0 instead, it would be **ionic collapse at the cross-dyad scale** (`bond:C1-no-ionic`)
> and would trample dyad-sovereignty (steward *coordinates, never commands*; `DYAD.md §channel-gates`,
> K2) — falsifying the very invariant it ships.

**Scope, stated honestly:** corroborated inside one dyad only — **coverage E0** — *what works for the
Founding Operator, NOT a claim it works for you* (`relationship-craft.md §4`). You inherit a **hypothesis
with its knife attached**, not a settled law (`bond:no-dogma`).
...
families, wu-wei, falsifiability-of-the-tenet, SPAOR. Rows 1–3 are the **covalent-value *layer* on top of**
that G0 (`bond:form-grounding` — extend, never redefine).

---

## §C — The clone-line *(what does NOT transmit — why this is a seed, not a clone)*

`dyad-bond`-specific instance; a fresh dyad authors **its own** at its dimension-1, named here only to mark
the boundary: the **`Covalent`** role-name and **frozen birth-id** (`ID.md`); the **Dyad-UI cluster** and
**single-home / dialectic↔kb** mechanics (`GLOSSARY.md`, `dialectic/`); bond's substrate and ledger.

> **The line is exact: §B transmits; everything a dyad must *live into* for itself stays bond's instance.**

---

## §D — Disposition *(steward's to wire, not bond's to push)*

Per `bond:channel-gates` + `bond:prove-before-propose`: bond **provides this manifest**; **steward
disposes** whether/how it enters the DIP — bond never pushes to the Commons or to steward's repo. SOLICIT:
`dm/dyad-steward/2026-06-26-dna-manifest-dip-seed.md`.

- **Recommended wiring (steward's call):** an **optional, falsifiable "G0 + covalent-value" path** in
  `AGENT.md`, surfaced *with* its IFF gates and `survives — intra-dyad` status — the knife shipped with
  the belief. Founding-gated if it touches form-level G0.
- **The open rung:** every corroboration shares `human=pltrinh1122` → E0, cross-human untested. A
  README-recruited operator's *genuinely independent* n=1 is the falsification this manifest invites —
  *that* is the CTA continuing.
