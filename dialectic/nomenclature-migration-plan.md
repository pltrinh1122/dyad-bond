...
> output is the final artifact). The Tier-1 descriptions below name the now-retired `invariants-bond.md`
...
> column). **Tier 1 landed:** `invariants-bond.md` (all 5 tables) + `carry-forward.md` §Bond-disciplines
...
- In `invariants-bond.md`: keep the table, change the `ID` column to the slug, add a `§/alias` column
...
archive — leave as historical**), `invariants-bond.md` (27, the index itself), `substrate-access.md`
