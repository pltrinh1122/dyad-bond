...
- **Spine (see the doc):** experiment subject = bond *autonomously* achieving grader-side convergence · 5-step acceptance discipline (⭐ test↔EXPECTED mapping-validation = the load-bearing judgment, CI is overbuild) · mapping failure-modes (worked: F-1.2 sort≠determinism, F-3 presence≠arming) · convergence determined in **retrospection/audit** (oracle-lagged, not oracle-less) · audit power ∝ §J independence · auditor architecture (3 conditions: independence-by-failure-class · sub-linear attention · shield-not-sword) · the s-local6 **autonomy-gap** (every load-bearing act was Operator-initiated → what the protocol exists to close).
