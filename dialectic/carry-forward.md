...
- **Session 2026-06-25 (s-resume — `resume` → `clarify IFF2` → the `it survives` correction → `land`)** (Covalent) — ephemeral cloud container, branch `claude/resume-4zo3b2`, `claude-opus-4-8`.
  ...
  - **➤ `rub-forward:`** (1) the whole `CP` cluster is `@candidate`, dog-fooded one session — does the displayed-`CP`/diligence-trace **reduce premature convergence across sessions** (Operator `raff`/`rub` pull-rate + length-trend), or become the 5th theater-venue? (2) `bond:no-self-act` still un-landed. (3) does "diligence" need an operationalized **rubric** (the moves that "bit") or stay judgment — the same define-it regress as "easy"/"resistance"?
