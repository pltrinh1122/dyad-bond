...
**Arc.** `d-start: recommend NBA per outstanding todos` → NBA recommended **A** (single-home Founding-gate PR) then **B** (dyad-system commission lint+pin); **both disposed Y, both turned out phantom:**
1. **A — single-home Founding PR was already merged** (PR #73, the-dyad-practice, 2026-07-01, merge `565790a`). Artifact live on the form's `main`. Todo (written 2026-06-28 at "staged, PR owed") never flipped → ~8-day phantom. Marked done; specialization residual (locus-axis + breadth falsifier, reuse-gated) re-filed. **PR #88-class false-open, not a fix.**
2. **B — dyad-system commission draft is doubly superseded.** Arc moved to the **Neutral Quarry** (`pltrinh1122/commission-dyad-system`; live `REQUIREMENTS.md` = REQ-of-record, **Model B / single-identity R3**; three-party pipeline running — PRs #4/#6/#8 merged 2026-07-06, #5 open). The local draft encodes the *reversed* pre-2026-07-05 `graduates-to` model and pins it "not negotiable." Lint PASSed (FORM-clean) but TRUTH-dead. **Did NOT pin/dispatch** (would re-inject the overturned model). Stamped the draft SUPERSEDED; reconciled the `in-progress` item to point at the quarry.

**Reflect landed** → `relationship-craft.md §Reflect — d-start NBA: two phantom recommendations, and the state-space-check regression` (full CSS+OR/SH shown in chat). **Headline STOP = ran the NBA off stale local do-state without verifying live state at recommendation-time; the one light check used `--state open`, missing a MERGED PR** — a direct regression on last session's own START (enumerate the state-space). **START = verify at recommendation-time + full state-space (`--state all`); structural fix = the external-drain-latency seam** (items resolving in external repos never flip the local ledger — NBA should mark external-terminating items unverified-until-checked, or stand-down must reconcile external resolution). **SH = none** (two routine Y's + the reflect close; both phantoms self-caught — 2nd consecutive zero-Operator-catch arc). **Ingraining-watch: n=2 on state-space-under-enumeration — a banked START that didn't fire next session → wants a mechanism, not another note.**

**Resume delta:** ledger is now honest — the two "concrete/near-terminal" NBA nodes were phantom and are cleared; the dyad-system arc correctly tracks to the **quarry** (`commission-dyad-system`), not the local draft. **The genuinely-open substantive fronts are untouched:** `C` relationship-craft codification (the deferred telos — 4th consecutive non-craft arc; NBA thesis was to propose 2–3 candidate bites, un-disposed) · `D` intent-clarity anchor-candidates · the parked cross-dyad/apex fronts. **Next-session caution:** the remaining `## todo` items have NOT been re-verified against external substrate — apply the START (verify live state before trusting the render). Session closed on Operator fatigue (`d-reflect: need rest`).
