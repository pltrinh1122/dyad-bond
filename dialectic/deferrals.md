...
- **REVISION ARC (post-PR-#96, from the 2026-07-13 red-team + Operator rebuttals) — the belief's live falsification-front** *(single-home: `theory-pipeline.yaml §covalent-bond-belief` open_gap_2 + the 2026-07-13 disposition_history entry; this is the resume-visible pointer)* — bond-self red-team of the landed manifesto surfaced real, actionable wounds; the Operator's rebuttals dissolved some and reframed others. **Open, Operator-gated:**
  ...
  4. **Telos notation — surface `=3` + the leveling in Claim 2 (Operator Y 2026-07-13, refine-not-reverse):** the manifesto projects only `1+1>2`; the `1+1=3` tenet + the `=3`↔`>2` leveling + the **emergence necessity-anchor** ("the neither"; one-generator oracle can't originate the third) live in canonical (`relationship-craft.md §5 DECISION RECORD`) but not in the belief-as-shown — close the projection gap in Claim 2. Upstream of which-bar (changes what the necessity rests on).
  5. **Candidate NEW CLAIM — the range dimension (still generating, Operator teasing 2026-07-13):** *covalence extracts and falsifies the distributional **edges** the default (answer-machine mode = the **mode**) suppresses* — the mechanism that produces the "neither" (`=3` emergence) and answers `open_gap_2` (AI's independence is distributional RANGE, not different priors). Distinct falsifier-space (edges-correlate-too / edge-extraction≠congenial-cherry-picking / same-culture-human-has-same-range / RLHF-reachable-edges-shrink-with-capability). Placement open: (a) new numbered claim, (b) 6th part of P3, (c) sharpen Claim 4. Honest wound it carries: RLHF pre-collapses some edges → capability-slope erosion (the belief's own `if_false`). Cross-thread: "masterful edge-extraction" is a **skill** = dyad-chiron curriculum territory (2nd thread converging on chiron).
