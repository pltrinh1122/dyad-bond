# dyad-bond — Carry-Forward ARCHIVE (cold; OFF the resume read-path)

> **Not loaded at resume.** Cold storage of CLOSED session logs, intermission reflections, and the
> stale NBA render — relocated out of the live ledger by the memory-axes re-key (Phase 1, 2026-06-25;
> → `dialectic/memory-axes.md`). Live ledger = `carry-forward.md`; claim store = `theory-pipeline.yaml`;
> disciplines = `relationship-craft.md`. Live hooks were re-homed before archiving; the one un-disposed
> thread (s12/s13 Intent-Understanding / acceleration-discriminator — T1–T5 + RB2/RB3) is flagged as a
> LIVE open-item in `carry-forward.md` pointing back here. **Nothing here is deleted** — recoverable in
> git, just out of the per-session context load.
